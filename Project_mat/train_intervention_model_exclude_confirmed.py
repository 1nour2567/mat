...
if '高血脂症二分类标签' in df.columns:
    print("\n高血脂症标签分布:")
    print(df['高血脂症二分类标签'].value_counts())
else:
    print("\n警告: 缺少高血脂症二分类标签列")

# 筛选未确诊的痰湿体质患者
# 体质标签为5表示痰湿质，高血脂症二分类标签为0表示未确诊
tan_shi_patients = df[(df['体质标签'] == 5) & (df['高血脂症二分类标签'] == 0)].copy()
print(f"\n未确诊的痰湿体质患者数: {len(tan_shi_patients)}")
...
output_path = '/workspace/Project_mat/data/processed/tan_shi_intervention_results_exclude_confirmed.csv'
