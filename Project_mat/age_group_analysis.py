...
print(f"各维度权重：")
print(f"Spearman R (痰湿): {w[0]:.4f}")
print(f"互信息 MI (风险): {w[1]:.4f}")
print(f"PLS 载荷 (联合): {w[2]:.4f}")
print(f"权重总和: {np.sum(w):.4f}")
...
print("\n关键指标排序（前15）：")
print(feature_scores.head(15)[['feature', 'feature_type', 'total_score']])
...
for i in range(1, 10):
    ...
    print(f"体质标签 {i} 的患病率: {prevalence:.4f}, 相对风险: {rr:.4f}")
...
# 排序
constitution_coefficients_sorted = sorted(zip(constitution_types, constitution_coefficients), 
...
plt.tight_layout()
plt.savefig('figure1_综合评分排序_年龄分组.png', dpi=300, bbox_inches='tight')
plt.close()
...
# 年龄组统计
group_stats = df.groupby('年龄组').agg({
    '高血脂症二分类标签': 'mean',
    '痰湿质': 'mean',
    'TC（总胆固醇）': 'mean',
    'TG（甘油三酯）': 'mean'
...
# 调整布局
plt.tight_layout()
plt.savefig('figure3_体质风险贡献_年龄分组.png', dpi=300, bbox_inches='tight')
plt.close()

# 图 4：关键指标与双目标的相关性热力图
print("生成图 4：关键指标与双目标的相关性热力图")
top10_features = feature_scores.head(10)['feature'].tolist()
heatmap_features = top10_features + ['痰湿质', '高血脂症二分类标签', '年龄组']
...
# 框出最后三列（痰湿质、高血脂标签和年龄组）
for i in range(n_rows):
    for j in [n_cols - 3, n_cols - 2, n_cols - 1]:
        rect = plt.Rectangle((j, i), 1, 1, fill=False, color='black', linewidth=2)
        ax.add_patch(rect)

plt.tight_layout()
plt.savefig('figure4_相关性热力图_年龄分组.png', dpi=300, bbox_inches='tight')
plt.close()

print("\n所有图表生成完成！")
print("已保存的图表：")
print("1. figure1_综合评分排序_年龄分组.png")
print("2. figure2_年龄分组分析.png")
print("3. figure3_体质风险贡献_年龄分组.png")
print("4. figure4_相关性热力图_年龄分组.png")
