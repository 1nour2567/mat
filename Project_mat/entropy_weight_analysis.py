...
bars1 = plt.bar(x - bar_width, top10['spearman_normalized'], width=bar_width, 
                label=f'Spearman R (痰湿) w={w[0]:.3f}', color='#264653', alpha=0.8)
bars2 = plt.bar(x, top10['mutual_info_normalized'], width=bar_width, 
                label=f'互信息 MI (风险) w={w[1]:.3f}', color='#2A9D8F', alpha=0.8)
bars3 = plt.bar(x + bar_width, top10['pls_normalized'], width=bar_width, 
                label=f'PLS 载荷 (联合) w={w[2]:.3f}', color='#8AC926', alpha=0.8)
...
# 图 4：关键指标与双目标的相关性热力图（使用熵权法排序的指标）
print("生成图 4：关键指标与双目标的相关性热力图（熵权法）")
...
# 添加标题
plt.title('图 4：关键指标与双目标的相关性热力图（熵权法）\n(Spearman 相关系数)', 
          fontsize=14, pad=20)
