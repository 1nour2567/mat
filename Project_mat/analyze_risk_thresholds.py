...
def print_threshold_basis():
    ...
    print("\n2. 高风险（中医预警高风险）:")
    print("   - 依据：模型预测概率在不确定区间 [0.35, 0.65]，且符合中医升档规则")
    print("   - 具体标准：痰湿质 ≥ 80 且 活动量表总分 < 40")
    
    print("\n3. 中风险:")
    print("   - 依据：模型预测概率在 [0.35, 0.65] 区间")
    print("   - 未触发中医升档或降档规则")
    
    print("\n4. 低风险:")
    print("   - 依据：模型预测概率 < 0.35")
    
    print("\n5. 低风险（中医支持）:")
    print("   - 依据：模型预测概率在不确定区间 [0.35, 0.65]，且符合中医降档规则")
    print("   - 具体标准：痰湿质 < 60 且 活动量表总分 ≥ 60")
    
    print("\n=== 阈值选取说明 ===")
    print("1. 血脂异常标准：严格执行赛题给出的临床阈值")
    print("2. 模型概率阈值：基于 ROC 曲线分析，0.35 为约登指数最优阈值")
    print("3. 中医规则阈值：")
    print("   - 升档阈值 (80分/40分)：参考赛题问题二示例中的高风险组合")
