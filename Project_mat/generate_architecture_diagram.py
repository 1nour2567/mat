...
def draw_rectangle(ax, x, y, width, height, text, facecolor='#E3F2FD', edgecolor='#1976D2', 
                   linewidth=2, text_color='#0D47A1', boxstyle='round,pad=0.1'):
    """绘制带文字的圆角矩形"""
...
def create_architecture_diagram():
    ...
    # 标题
    ax.text(50, 57, '三层融合预警模型架构流程图',
            ha='center', va='center', fontsize=16, fontweight='bold',
            color='#1565C0')
    
    # 子标题
    ax.text(50, 54, '中西医结合 · 特征隔离 · 分层预测',
    ...
    # 低风险（绿色）
    low_risk = draw_rectangle(ax, 15, output_y, 18, output_height,
                            '低风险',
                            facecolor='#C8E6C9', edgecolor='#2E7D32',
                            text_color='#1B5E20', boxstyle='round,pad=0.3')
    
    # 中风险（黄色）
    mid_risk = draw_rectangle(ax, 41, output_y, 18, output_height,
                            '中风险',
                            facecolor='#FFF9C4', edgecolor='#FBC02D',
                            text_color='#F57F17', boxstyle='round,pad=0.3')
    
    # 高风险（红色）
    high_risk = draw_rectangle(ax, 67, output_y, 18, output_height,
                             '高风险',
    ...
    # 第三层 → 输出层
    draw_arrow(ax, 35, layer3_y, 24, output_y + output_height, color='#2E7D32')
    draw_arrow(ax, 50, layer3_y, 50, output_y + output_height, color='#FBC02D')
    draw_arrow(ax, 65, layer3_y, 76, output_y + output_height, color='#D32F2F')
    
    # 第一层直接 → 高风险（绕过第二、三层）
    # 画一条弯曲的箭头
    arrow1 = FancyArrowPatch((27, layer1_y + layer_height/2), (76, output_y + output_height),
    ...
    # 血脂特征说明（红色小方块）
    red_patch = Rectangle((legend_x, legend_y), 2, 1.5, facecolor='#FFEBEE', edgecolor='#D32F2F')
    ax.add_patch(red_patch)
    ax.text(legend_x + 3, legend_y + 0.75, '血脂异常直接确诊',
            ha='left', va='center', fontsize=8, color='#B71C1C')
    
    plt.tight_layout()
    
    # 保存图片
    output_path = '/workspace/Project_mat/三层融合预警模型架构图.png'
