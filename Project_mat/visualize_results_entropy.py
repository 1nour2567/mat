...
# 计算各维度的变异系数
cv_r = np.std(spearman_normalized) / np.mean(spearman_normalized)
cv_m = np.std(mutual_info_normalized) / np.mean(mutual_info_normalized)
cv_l = np.std(pls_normalized) / np.mean(pls_normalized)

print(f"R'（痰湿表征）的变异系数: {cv_r:.4f}")
print(f"M'（风险预警）的变异系数: {cv_m:.4f}")
print(f"L'（联合载荷）的变异系数: {cv_l:.4f}")

# 归一化权重
cv_sum = cv_r + cv_m + cv_l
weight_r = cv_r / cv_sum
weight_m = cv_m / cv_sum
weight_l = cv_l / cv_sum

print(f"\n归一化权重:")
print(f"R'（痰湿表征）: {weight_r:.4f}")
print(f"M'（风险预警）: {weight_m:.4f}")
print(f"L'（联合载荷）: {weight_l:.4f}")

# 修正后的综合评分
weights = [weight_r, weight_m, weight_l]
...
# 添加标题和图例
plt.title(f'图 1：双目标联合筛选综合评分排序图（熵权法）\nF = {weight_r:.2f}×R\' + {weight_m:.2f}×M\' + {weight_l:.2f}×L\'', 
          fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig('figure1_综合评分排序_熵权法.png', dpi=300, bbox_inches='tight')
plt.close()

# === 图 2：三维评分分解图（分组柱状图）===
print("生成图 2：三维评分分解图（熵权法）")
...
plt.title('图 2：三维评分分解图（熵权法）', fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig('figure2_三维评分分解_熵权法.png', dpi=300, bbox_inches='tight')
plt.close()
...
# 调整布局
plt.tight_layout()
plt.savefig('figure3_体质风险贡献_熵权法.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('figure4_相关性热力图_熵权法.png', dpi=300, bbox_inches='tight')
plt.close()

print("\n所有图表生成完成！")
print("已保存的图表：")
print("1. figure1_综合评分排序_熵权法.png")
print("2. figure2_三维评分分解_熵权法.png")
print("3. figure3_体质风险贡献_熵权法.png")
print("4. figure4_相关性热力图_熵权法.png")
